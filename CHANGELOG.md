...
- **Trie prune race against concurrent commits closed** (PR #711). `maybe_prune_trie` spawns the prune on a background `std::thread` cloning the trie. The cloned trie's `self.version` is frozen at clone time. While the live-set walk above runs (seconds to minutes), the main thread keeps applying blocks; each commits a new root via `flush_trie_batch` with its own `value_hash` / `node_hash` entries. Those new hashes were NOT in the live set the prune built from the snapshot. `gc_orphaned_nodes` then deleted them as orphans. Symptom: validator A core hit orphan-value `516a0dc27c…` at h=2215224 (chain.db corruption within milliseconds of finalising that block), validator B fullnode-1 hit orphan-node `516a0dc27c…` at h=2131563, validator B fullnode-2 hit orphan-node `28fdc024ed1d…` at h=2125714 — different roles, different heights, same race class. Fix: re-load `latest_version()` right before `gc_orphaned_nodes` and walk every root committed after `self.version` to augment the live set. Shrinks the race window from "full walk duration" to "between the final reload and the gc commit" — at most one block's worth of writes. A truly race-free fix requires running the live walk and the delete inside the same MDBX RW transaction; that refactor (exposing the txn handle through `TrieStorage`) is bigger surface and belongs in its own PR. Regression test `test_prune_augment_walk_preserves_newer_versions_committed_during_race` simulates the race with a frozen prune snapshot at v=3 and a concurrent v=4 commit; asserts the v=4 value hash survives the prune.
- **B3b — recompute `total_minted` from blocks when blob is stale** (PR #712). `load_blockchain` runs Patch B3 to overwrite stale `accounts` from the canonical trie. But `total_minted` lives only in the bincode blob — no trie key, no per-block index. So when B3 fires (trie ahead of blob), `accounts` snap to canonical but `total_minted` stays at whatever the last save_blockchain wrote. Symptom: two validators ended up with identical `accounts` (B3 reconciled both from the same trie) but differing `total_minted`. `STATE-FP fp = SHA256(acc + total_minted)` then disagreed across two otherwise-identical validators. Same block content, same state-root in the trie, but the fingerprint trace fired. Fix: `recompute_total_minted_from_blocks(bc)` walks every persisted block 1..=tip, sums `block.coinbase().amount`, and adds `TOTAL_PREMINE`. C-04 (`block_executor.rs:336`) enforces `coinbase.amount == reward`, so the chain has no other source of newly-minted supply — this sum is the canonical value at the tip. `load_blockchain` compares blob vs recomputed and warns + overwrites on mismatch; save-back happens via the existing atomic B1 path. Cost: O(N) MDBX block loads at boot (~30–60s at mainnet h≈2.2M on warm SSD). Three regression tests cover the helper ground truth, the end-to-end `load_blockchain` repair path, and the missing-block fail-soft posture.
- **`collect_reachable` empty-subtree check uses the right depth** (PR #714). The GC live-set builder was checking `hash == empty_hash(0)` (root-level sentinel) instead of the depth-specific `empty_hash(depth)`. Empty subtrees at depth > 0 are never materialised in `trie_nodes`, so the walk fell through to `load_node` and got `Ok(None)`. Silently disagreed with `walk_verify` (`tree.rs:571`), which has always taken depth. Fix: pass `depth` parameter, check `hash == empty_hash(depth)`, recurse with `depth + 1`. Defensive consistency for the live-set bookkeeping rather than a directly load-bearing fix for the production symptom (PR #711 closes the symptom proper).
- **`pay_one_signer` iterates sorted delegator keys** (PR #713). `pay_one_signer` was the last reward-path function still iterating `self.delegations.values()` directly. `slash()` was hardened in PR #551 to sort delegator keys before iterating; the reward-distribution path missed the same treatment. Lifts the sorted-keys pattern from `slash()` (line 556) into `pay_one_signer`. Per-share math is already order-independent (each share is computed from full `delegator_pool` and full `total_delegated`, then aggregated via associative `saturating_add`), so this is NOT a proven cause of the 2026-05-24 `fp` drift — that root cause sits in PR #712's B3b. This is structural parity that closes the "two functions in the same crate reasoning about HashMap iteration order differently" mismatch.
- **Workspace version bump 2.2.15 → 2.2.16** (this PR).

Known follow-up: a truly race-free trie prune requires running the live walk and the GC delete inside a single MDBX RW transaction. PR #711 narrows the window to "between final reload and gc commit"; the B3b path in PR #712 self-heals divergence on boot as defense in depth. A larger refactor exposing the MDBX txn handle through `TrieStorage` is deferred — it changes a load-bearing surface and warrants its own PR with a dedicated review.
...
## [2.1.80] — 2026-05-07 — audit closure batch (5 HIGH + 7 MEDIUM + 3 LOW)

Bundles the 2026-05-06 rust-workspace security audit closures into one mainnet binary. State-affecting fork changes ship gated behind env vars defaulting to `u64::MAX`, so v2.1.80 is bit-identical to v2.1.79 on chain history until each fork is activated separately.

- **H1 + H2** (PR #511) — `execute_evm_tx_in_block` no longer silent-defaults `TxEnv::builder().build().unwrap_or_default()` (ran a zero-env tx through revm with non-deterministic results across builds) or `gas_limit.parse().unwrap_or(30_000_000)` (let one tx consume the entire block budget, bypassing `TX_GAS_LIMIT_CAP`). Both paths now log + `mark_evm_tx_failed` + return `Ok(())`; block still progresses, receipt correctly reports `status=0x0`. Identity for every valid tx coming through RPC ingress.
- **L2** (PR #514) — same shape, sibling early-returns: malformed RLP hex in signature field + `TxEnvelope::decode_2718` failure now `mark_evm_tx_failed` instead of silent succeed-with-`status=0x1`.
- **M1 + M5** (PR #512) — receipt + log + tx-index MDBX put errors no longer silently swallowed; emit `tracing::warn!` so a failed-to-persist receipt surfaces in ops dashboards instead of becoming a transient `eth_getTransactionReceipt` 404.
- **L1, L3** (PR #512) — delete dead `let _sender = recover_signer().ok();` (wasted ~100µs/tx of secp256k1 work) and replace `unreachable!("TokenOp variant handled above")` with `Err(InvalidTransaction)` so a future variant addition doesn't `panic!` + abort the validator.
- **M6** (PR #515) — `mempool_txids: HashSet<String>` + `mempool_sender_count: HashMap<String, u32>` sidecars on Blockchain. `add_to_mempool`'s duplicate-txid scan + 2× `mempool_pending_count` (~25M comparisons per block at 10K-mempool / 5K-burst) collapse to O(1) lookups. Sidecars `#[serde(skip)]`, rebuilt from the authoritative `mempool` VecDeque after `retain` / snapshot rollback / post-storage-load. Zero state/consensus impact.
- **M2** (PR #517) — reorder `execute_evm_tx_in_block` Ok-arm: AccountDB state mutations (EIP-170 contract code + state writeback) now complete BEFORE MDBX persists (receipt + logs). A panic mid-tx now leaves state-applied with no receipt (refillable via peer gossip) instead of receipt-with-no-state (required full chain.db rsync). End-state in happy path identical regardless of ordering.
- **M3** (PR #518) — `slash()` preserves the `total_delegated == sum(entries.amount)` invariant. Pre-fix the validator-level subtraction ran BEFORE the per-entry ceiling-division loop; ceiling rounding could over-slash by 1 sentri per delegator entry, drifting the validator's recorded `total_delegated` by N sentri (N = delegator count) per slash and inflating `pay_one_signer`'s denominator → delegators slowly under-paid. Now: per-entry slash → sum survivors → assign `total_delegated = sum`. Protocol-wide ≥ stated-rate guarantee preserved.
- **M4** (PR #519) — `check_liveness` calls `is_downtime_at(validator, current_height)` instead of the entry-count `is_downtime`. Pre-fix a validator with 0 signed entries had `0 < 14400 → false` → never flagged for downtime; complete non-participation was immune to the auto-jail path.
- **M7** (PR #525) — `on_round_status_weighted` always refreshes peer stake; only the round update is gated on `>=`. A peer crossing an epoch boundary (stake delta) without round advance kept its pre-epoch cached stake, and `f_plus_one_round`'s stake-weighted threshold summed stale values.
- **H4** (PR #522, layers 1+2) — surface `process_double_sign` Err (was `let _ = …`, silently succeeded) AND split submitter / offender on `StakingOp::SubmitEvidence` (added `offender: String` field with `#[serde(default)]` for back-compat). Pre-fix the apply path forced `evidence.validator = tx.from_address` — a third-party reporter ended up accusing themselves. Layer 3 (full sig verification of `signature_a` / `signature_b` against `Precommit::signing_payload`) deferred — the wire format doesn't carry `round_a` / `round_b` yet, separate fork-gated extension.
- **H5** (PR #520) — peek-then-transfer-then-drain on `StakingOp::ClaimRewards`. Pre-fix drained both reward accumulators BEFORE attempting the `PROTOCOL_TREASURY → claimer` transfer; if the transfer Erred, Pass-2 rollback restored `accounts` from `BlockchainSnapshot` but the snapshot doesn't capture `stake_registry` — accumulators stayed zero, claimer's rewards permanently lost. New non-mutating `peek_delegator_rewards` getter; drain only fires after transfer succeeds.
- **H3** (PR #532, **fork-gated**, `EVM_GAS_FIX_HEIGHT` env, default `u64::MAX`) — closes the EVM gas supply leak. Pre-fix the write-path EVM tx had revm internally deduct `gas_used × INITIAL_BASE_FEE` from sender wei; the writeback's wei→sentri floor-div silently dropped that delta. Post-fork sets `cfg.disable_base_fee = true` + `gas_price = 0` so revm's internal gas accounting is zero; Pass-1 native flat tx.fee (10K sentri, 50/50 burn/validator) is the entire fee. Reproducer `test_h3_post_fork_supply_invariant_holds_on_value_transfer` in `evm_e2e_harness.rs` asserts `sender_drop == value + tx.fee` exact; mainnet activation deferred — operator flips after testnet bake confirms the supply-conservation invariant on real flows.
- **L6 + L7** (PR #533, doc-only) — strong warning that `is_sentrix_precompile` returning `true` is NOT proof of a wired handler (0x0100 / 0x0101 reserved-but-unwired today). `#[deprecated]` on legacy `is_downtime` + `record(_, _, signed)` after M4 migration; `#[allow(deprecated)]` on the test module so kept-for-migration tests still compile.
- **EVM e2e harness #2** (PR #516) — fills audit's untested-fn #2 reproducer. Real `alloy_consensus::TxLegacy` + secp256k1 signing → `alloy_primitives::Signature` translation → `TxEnvelope::Legacy` → EIP-2718 encode → Sentrix wire format Tx → mempool admit → block apply → assert nonce bump + balance debit + receipt persistence to TABLE_RECEIPTS.
- **AccountDB supply-invariant proptest** (PR #513) — random sequences of `credit` / `charge_fee_only` / `transfer` against AccountDB; independent ledger model maintains expected state; 256 cases per run. Plus an explicit H3-hazard regression-guard test asserting `set_balance` DOES break the invariant today.
- **deny.toml duplicate-version cleanup** (PR #524) — silenced 14 cargo-deny `multiple-versions = "warn"` entries plus dropped 3 stale advisory ignores that no longer match any in-tree crate version. `cargo deny check` reports zero warnings on stable.
- **Operator infrastructure** (PRs #521 + #523) — owner-auto-merge workflow (filters on `user.login == 'satyakwok'`) + external-PR security heuristics gate (workflow modification, encoded blob in diff, sensitive config touched, suspicious networking host).

Built via `rust:1.94-bullseye`, sha256 `6304381a96cd04a2819bff390630201e6d25e3024731f0fdde85ce79053760bb`. Deployed cluster-wide via halt-all + canonical chain.db rsync (cluster-state recovery had run twice the same evening due to recurring state_root v2 drift, root-cause investigation ongoing — see v2.1.81 below).
...
## [2.1.77] — 2026-05-07 — STATE_ROOT_V2 canonical-override (Option B) + extended historical-state honesty across 5 RPC methods

Two related correctness improvements following the v2.1.76 STATE_ROOT_V2 deploy and its first activation attempt that surfaced silent in-memory drift across validators.

- **Option B canonical PROTOCOL_TREASURY rebase** (PR #489) for safe `STATE_ROOT_V2` activation. v2.1.76 shipped the touch-list addition (PROTOCOL_TREASURY in the trie's `touched_addrs` past `STATE_ROOT_V2_HEIGHT`), but the first activation attempt forked the cluster: each validator inserted its own drifted in-memory PROTOCOL_TREASURY balance into the trie, producing competing state_roots with no 3-of-4 majority. The drift had been silent for ~700K blocks because the very bug v2 was meant to fix prevented consensus from detecting it. v2.1.77 adds an env-driven canonical override — when `STATE_ROOT_V2_TREASURY_BALANCE=<sentri>` is set alongside `STATE_ROOT_V2_HEIGHT=<h>`, the activation block force-sets the in-memory PROTOCOL_TREASURY balance to that operator-picked canonical before the trie commit. All validators with the same env converge on a single trie root. Operator runbook embedded in `crates/sentrix-core/src/blockchain.rs::update_trie_for_block`. Fires only at the exact activation block; subsequent blocks evolve the trie normally as coinbase mints + ClaimRewards drive treasury growth. Three unit tests pin the rebase semantics. Mainnet activation 2026-05-07 at `h=1638700` succeeded — pre-rebase drift of 1-5 SRX across nodes was surfaced and corrected by the canonical override; state_root flipped lockstep across all four validators within 70ms. **Post-activation note:** drift re-emerges over time after activation (root cause not yet identified — likely coinbase reward depending on locally-tracked liveness state, or other system-account non-determinism). Operators should expect periodic chain.db rsync recoveries (~10min MTTR) until the underlying drift mechanism is narrowed and fixed. Trade-off vs the pre-fix silent freeze: drift is now visible (cluster halts on fork rather than diverging silently forever) — this is net positive for state_root commitment correctness, but adds operational toil until root cause is fixed.
- **Extended historical-state honesty pattern from `eth_getBalance` to all five state-read methods** (PR #489 second commit). v2.1.76 fixed `eth_getBalance` to return `-32004 "historical state reads not yet supported; use 'latest'"` for specific historical block tags instead of silently serving latest state. The same silent-stale issue existed in `eth_getCode`, `eth_getStorageAt`, `eth_call`, `eth_getTransactionCount`. Factored the height-check into a `require_latest_state_read` helper in `crates/sentrix-rpc/src/jsonrpc/helpers.rs` and applied it consistently. `pending` continues to add mempool count to nonce. Block-content methods (`eth_getBlockByNumber`, `eth_getBlockByHash`, etc.) intentionally NOT gated — historical block bodies live in chain.db and serve correctly, only the account-state subset is restricted. Surveyed downstream consumers (indexer, frontends, explorer) for callers passing specific block tags to state-read methods — zero hits, fully backward-compatible. Ten unit tests in `helpers.rs::tests` lock down each branch of the gate.

Build via `rust:1.95-bullseye` for glibc 2.31 baseline. Mainnet halt-all + simul-start window: 12 seconds. Activation height crossing: ~6 minutes after restart, lockstep flip at `0xc601ec5f...` → `0x9db520b7...`.

## [2.1.76] — 2026-05-06 — `eth_getBalance` height awareness + dormant STATE_ROOT_V2 fix

Two correctness improvements bundled out of the 2026-05-05 transport audit. Bug A is pure RPC additivity; Bug B is consensus-touching and ships dormant.

- **`eth_getBalance` height awareness** (PR #479). Pre-fix, the handler read `params[0]` (address) but never `params[1]` (block tag) — historical reads silently returned the latest balance. Audit surfaced the gap when probing balance at `h=1M` / `h=1.5M` / `h=1.62M` all returned identical hex. Fix: tags `latest` / `earliest` / `pending` / `finalized` / `safe` continue to return current state; specific block-number params return `-32004 "historical state reads not yet supported"`. Callers see the gap loudly instead of consuming stale data.
- **STATE_ROOT_V2 fix shipped DORMANT** (PR #479). The state_root has been frozen since `h~1.253M` because `update_trie_for_block` never included `PROTOCOL_TREASURY` (`0x...0002`) in its `touched_addrs`. Coinbase txs render `to_address = <validator>` for display, but per the V4 reward v2 fork (active since `h=590100`) the actual state mutation routes the mint to PROTOCOL_TREASURY. The trie never observed the change, so the state_root froze the moment quiet coinbase-only blocks became the steady state. Fix: gate-activated by `STATE_ROOT_V2_HEIGHT` env var (default `u64::MAX` = dormant). Past activation, append PROTOCOL_TREASURY to the touch list so the trie re-roots correctly. **Note:** the simple touch-list addition is necessary but not sufficient on its own — see v2.1.77's Option B canonical override for the complete activation mechanism.

## [2.1.75] — 2026-05-05 — `net_peerCount` + `web3_sha3` + `eth_getBlockTransactionCountByNumber` JSON-RPC

Three small additions to the JSON-RPC surface, surfaced by a transport audit. Wagmi / ethers / cast dispatch these on routine wallet flows; absence triggered `MethodNotFoundRpcError` in some clients.

- **`net_peerCount`** (PR #478) — returns active validator count minus self as a hex string. The libp2p Swarm handle isn't yet threaded through SharedState (would be a deeper refactor), so this is a best-effort proxy honest enough for the wallets that look at the value.
- **`web3_sha3`** (PR #478) — keccak256 hash of a hex-encoded byte string. Wagmi / ethers utility tier occasionally calls this for client-side hashing parity.
- **`eth_getBlockTransactionCountByNumber`** (PR #478) — common ethers / wagmi probe for "how busy is this block." Mirrors the same block-resolution path as `eth_getBlockByNumber`.

## [2.1.74] — 2026-05-04 — inbound libp2p watchdog closes silent-peer halt class

Validators occasionally entered a state where `libp2p_node.health_check()` reported all peers connected and BFT broadcasts succeeded, but the in-process libp2p task had silently stopped servicing the inbound side of the swarm — peer requests landed in the kernel's TCP receive buffer and were never read. Outgoing votes went out fine; incoming responses never came back. Chain stalled at `BFT round start` for 60+ seconds.

Fix (PR #477): a watchdog inside the libp2p task that records the timestamp of the last successfully-processed inbound swarm event. When that timestamp ages past `INBOUND_STALL_THRESHOLD = 60s`, the task aborts cleanly with `LIBP2P_INBOUND_STALL` so systemd `Restart=always` brings it back. Recovers within ~5s of the threshold trip vs the 90s `validator_watchdog` chain-height-stuck threshold (which would also fire eventually but later). Operator-facing: nothing changes in the happy path; in the silent-stall pattern, the journal now shows a clear `LIBP2P_INBOUND_STALL` line right before the restart, distinguishing it from generic chain stalls.
