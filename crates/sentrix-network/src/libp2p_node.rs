...
async fn run_swarm(
    ...
    // 2026-05-11: inbound-silence watchdog removed. Under cascade
    // conditions BFT phases legitimately go quiet for tens of seconds
    // (validators voting nil while waiting for prevote/precommit
    // supermajority over WAN); the 30-second threshold force-disconnected
    // peers mid-cascade, libp2p re-handshake then dropped enough BFT
    // messages to push the next round into the same failure mode —
    // self-reinforcing loop. Stall recovery is the external supervisor's
    // job same as swarm-task watchdog (removed earlier).
